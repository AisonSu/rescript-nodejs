...
zora("Fs", t => {
  t->test("readFile should read entire file", t => {
    ...
    ->then(
      fh => FileHandle.readFile(fh)->then(buffer => FileHandle.close(fh)->then(_ => done(buffer))),
    )
    ->then(
      buffer => {
  ...
  t->test("readFileWith should read entire file as a string", t => {
    ...
    ->then(
      fh =>
        FileHandle.readFileWith(fh, readFileOptions(~encoding="UTF-8", ()))->then(
          buffer => FileHandle.close(fh)->then(_ => done(buffer)),
        ),
    )
    ->then(
      content => {
